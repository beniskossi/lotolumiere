...
import { log, selectWithRandomization } from "../_shared/utils.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    log("info", "Generating predictions for storage", { drawName });
    ...
    // Vérifier le cache
    const cacheKey = `historical_${drawName}`;
    let historicalData: HistoricalDraw[] | null = dataCache.get(cacheKey);
    ...
    if (!historicalData) {
      ...
      if (fetchError) {
        log("error", "Error fetching data", { drawName, error: fetchError });
        throw fetchError;
      }

      historicalData = (data || []) as HistoricalDraw[];
      
      // Mettre en cache (10 minutes)
      dataCache.set(cacheKey, historicalData, 10 * 60 * 1000);
    }
    ...
    log("info", "Analyzing historical draws", { drawName, count: historicalData.length });

    // Multi-model approach
    const frequencyPrediction = predictByFrequency(historicalData);
    const sequencePrediction = predictBySequence(historicalData);
    const gapAnalysisPrediction = predictByGapAnalysis(historicalData);
    ...
    const frequencyConfidence = calculateConfidence(historicalData, frequencyPrediction);
    const sequenceConfidence = calculateConfidence(historicalData, sequencePrediction);
    const gapAnalysisConfidence = calculateConfidence(historicalData, gapAnalysisPrediction);
    const hybridConfidence = calculateConfidence(historicalData, combinedPrediction);
    ...
    if (insertError) {
      log("error", "Error inserting predictions", { drawName, error: insertError });
      throw insertError;
    }

    log("info", "Predictions stored successfully", { drawName, count: predictions.length });
    ...
    log("error", "Prediction generation error", { error });
...
// LightGBM-like: Weighted frequency analysis
function predictByFrequency(data: HistoricalDraw[]): number[] {
  const frequencies = new Map<number, number>();
  const recentWeight = 0.92;
...
// CatBoost-like: Pattern sequence analysis
...
function predictByGapAnalysis(data: HistoricalDraw[]): number[] {
  ...
  for (let num = 1; num <= 90; num++) {
    ...
    const consistencyBonus = variance < 5 ? 1.4 : 1.0;
...
function calculateConfidence(data: HistoricalDraw[], prediction: number[]): number {
  ...
  recentDraws.forEach((draw, index) => {
    const weight = Math.pow((index + 1) / recentDraws.length, 1.2);
  ...
  const totalConfidence = baseConfidence + frequencyScore + recencyScore;
